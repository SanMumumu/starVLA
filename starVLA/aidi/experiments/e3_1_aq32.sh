#######
# 中文注释：E3.1（action query num 消融，与 E1.2 对齐：四任务、无 CED）—— 本实验 n_action_query=32。
# 仅放大 action query token 数（conditioning 容量，与 chunk=8 解耦），其余与 E1.2 完全一致。
# baseline=16（E1.2 默认/隐式 baseline，不单列）；本系列从 32 往上扫 32/64/128。
#######
CONFIG=examples/LIBERO/train_files/starvla_jointflow_ced_libero.yaml
RUN_NAME=jf_e3_1_aq32
OVERRIDES=(
  ...
  --framework.action_model.n_action_query=32
)
