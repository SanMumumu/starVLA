# RoboTwin AIDI 8-GPU Evaluation

This folder runs RoboTwin evaluation on AIDI:

- 1 server job: starts 8 policy servers.
- 1 client job: starts 8 RoboTwin clients and aggregates results.
...
```bash
cd examples/Robotwin/eval_horizon
...
```yaml
RUN_SCRIPTS: "${WORKING_PATH}/starVLA/examples/Robotwin/eval_horizon/run_aidi_server.sh"
```
...
DEFAULT_ROBOTWIN_PATH="/path/to/RoboTwin"
...
clean2rand and rand2rand use the same test setting. Both should evaluate with `demo_randomized`; the difference is the training data.
...
```text
<model_root>/robotwin_eval_results_<run_name>_8shard/
```
